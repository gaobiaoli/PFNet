...
def class_metrics(label_True,label_pred):
    metrics={}
    hist=confusion_matrix(label_True.flatten(),label_pred.flatten())
    metrics["Accuracy"]=np.sum(np.diag(hist)) / hist.sum()
    metrics["Precision"]=np.diag(hist) / np.sum(hist, axis=0)
    metrics["Recall"] = np.diag(hist) / np.sum(hist, axis=1)
    return metrics
